...
public class BetterList<T>
{
	...
	private void AllocateMore()
	{
		...
		{
			((global::System.Array)buffer).CopyTo((global::System.Array)array, 0);
		}
		buffer = array;
	}
	...
	public bool Contains(T item)
	{
		...
		for (int i = 0; i < size; i++)
		{
			if (((object)buffer[i]/*cast due to .constrained prefix*/).Equals((object)item))
			{
				return true;
			}
		}
	...
	public bool Remove(T item)
	{
		if (buffer != null)
		{
			EqualityComparer<T> val = EqualityComparer<T>.Default;
			for (int i = 0; i < size; i++)
			{
				if (val.Equals(buffer[i], item))
				{
					size--;
					buffer[i] = default(T);
