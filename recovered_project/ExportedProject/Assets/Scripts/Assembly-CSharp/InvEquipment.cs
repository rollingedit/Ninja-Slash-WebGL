...
public class InvEquipment : MonoBehaviour
{
	...
	public InvGameItem Replace(InvBaseItem.Slot slot, InvGameItem item)
	{
		...
		if (slot != InvBaseItem.Slot.None)
		{
			...
			for (int num2 = mAttachments.Length; i < num2; i++)
			{
				...
				GameObject val = invAttachmentPoint.Attach((invBaseItem == null) ? null : invBaseItem.attachment);
	...
	public InvGameItem Equip(InvGameItem item)
	{
		if (item != null)
		{
			...
			Debug.LogWarning((object)string.Concat((object)"Can't resolve the item ID of ", (object)item.baseItemID));
		}
		return item;
	}
