...
public class UIPanelAlpha : MonoBehaviour
{
	...
	private void Start()
	{
		mColliders = ((Component)this).GetComponentsInChildren<Collider>(true);
		mWidgets = ((Component)this).GetComponentsInChildren<UIWidget>(true);
		if (mWidgets.Length == 0)
		{
			Debug.LogError((object)"Expected to find widgets to work with", (Object)(object)this);
	...
	private void UpdateAlpha()
	{
		...
		if (mLevel == 0)
		{
			...
			{
				NGUITools.SetActive(((Component)transform.GetChild(j)).gameObject, true);
			}
			int k = 0;
		...
		{
			TweenColor[] componentsInChildren = ((Component)this).GetComponentsInChildren<TweenColor>();
			int l = 0;
		...
		if (alpha < 0.01f)
		{
			...
			{
				NGUITools.SetActive(((Component)transform2.GetChild(n)).gameObject, false);
			}
			mLevel = 0;
		}
		else if (alpha > 0.99f)
